...
class FlashAttention(nn.Module):
    ...
    def flash_attention_forward(self, Q, K, V, mask=None):
        if mask is not None and rearrange is None:
            raise ImportError("einops is required only when FlashAttention is used with masking.")
        BLOCK_SIZE = 32
...
class ReformerLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        if LSHSelfAttention is None:
            raise ImportError("reformer_pytorch is required only when ReformerLayer is used.")
