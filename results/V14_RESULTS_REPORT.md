...
## 11. Architecture — Backbone-Pluggable Refactor (2026-05-10)

§8 의 layout 은 V14 vintage 산출 시점의 single-backbone (iTransformer hardcoded) 구조. 이후 코드 organization 만 **backbone-pluggable** 형태로 전환됨 — V14 평가 결과(§4)와 production 권장(§9)은 동일하게 유지되며, **PSM 회귀 테스트로 bit-for-bit identical 검증 완료** (`bundle_meta.json`, `vus_pr_D_w`, `vus_pr_D_w_z` 등 전 12 metric Δ = 0).

### 11.1 변경 동기

FDAS 의 본질적 가치 — *"multi-horizon forecaster 어떤 것이든 그 위에 FDAS 를 얹으면 anomaly score 로 동작한다"* — 를 paper-level 로 실증하려면 iTransformer 외 forecasting backbone (DLinear, PatchTST, TimesNet 등) 비교가 필요. 02–07 에 hardcoded 됐던 iTransformer 의존을 **registry pattern** 으로 추상화.

### 11.2 새 코드 구성

```
V13/
├── model/                            ← (NEW) backbone registry
│   ├── __init__.py                   — BACKBONES dict + get_backbone()
│   ├── base.py                       — BackboneSpec dataclass
│   │                                   (model_factory, default_model_hps,
│   │                                    default_training_hps, forward_signature)
│   └── iTransformer.py               — V14 의 production backbone (코드 무수정)
├── layers/                           — TSL standard (무수정)
├── scripts/
│   ├── 01_data_preparation.py        — backbone-agnostic (무수정)
│   ├── 02_train.py                   — `--backbone <name>`, registry dispatch
│   ├── 03_inference.py               — `--backbone <name>`, factory 로 model 재구축
│   ├── 04_score_compute.py           — `--backbone <name>`, per-backbone path
│   ├── 05_metrics.py                 — `--backbone <name>`, per-backbone CSV
│   ├── 06_cross_dataset.py           — `--backbone <name>`
│   ├── 07_visualization.py           — `--backbone <name>`, per-backbone figures
│   ├── run_all.py                    — `--backbone` 을 모든 단계에 전파
│   ├── config_factory.py             — (NEW) 02/03 공통 Config 빌더 +
│   │                                          train_config 직렬화 / 복원
│   ├── score_utils.py                — FDAS 핵심 수식 (무수정),
│   │                                   `has_required_*` / `load_*` 에 backbone 인자
│   ├── artifact_paths.py             — path helper 들에 backbone 인자
│   └── migrate_to_backbone_layout.py — (NEW) 일회성 mv 스크립트
├── models/<dataset_key>/<backbone>/  ← (CHANGED) per-(dataset, backbone) 격리
│   ├── best_model.pth
│   ├── checkpoint.pth
│   ├── train_config.json             — backbone 필드 박힘 (재구축에 사용)
│   └── training_history.json
├── results/<dataset_key>/
│   ├── bundle_meta.json              — dataset-level (backbone-agnostic)
│   └── <backbone>/                   ← (CHANGED) per-backbone artifact
...
│       └── test_labels.npy
├── results/04_metrics/
│   ├── per_dataset_metrics__iTransformer.csv     ← (NEW: per-backbone suffix)
│   └── metrics_tsb_format__iTransformer.csv
├── results/05_cross_dataset/<backbone>/          ← (NEW: per-backbone)
└── results/figures/<backbone>/                   ← (NEW: per-backbone)
```

기존 V14 vintage 의 200 데이터셋 artifact 는 `migrate_to_backbone_layout.py` 가 일회성으로 새 layout (`models/<key>/iTransformer/`, `results/<key>/iTransformer/`) 으로 이전 — **2200 file mv, leftover 0, 재학습 0**.

### 11.3 BackboneSpec 인터페이스
...
class BackboneSpec:
    ...
    default_model_hps: dict          # d_model, n_heads, e_layers, ...
    default_training_hps: dict       # lr, batch_size, num_epochs, patience, optimizer, scheduler
    extra_config_fields: list[str]   # train_config.json 에 박을 필드
    forward_signature: str = "tsl"   # "tsl" or "x_only"
```

각 backbone 은 **자기 paper 의 권장 HP** 를 그대로 박아둠 (통일된 학습 setup 강제 X) — "각 backbone 이 best 일 때도 FDAS 가 잔차보다 효과적인가" 라는 더 강한 narrative 를 위해.

### 11.4 새 backbone 추가 절차

1. `V13/model/<name>.py` 작성 — `Model(configs)` class, TSL forward signature: `(x_enc, x_mark_enc, x_dec, x_mark_dec) → Tensor[B, H, C]`. 30 ~ 200 줄.
2. `V13/model/__init__.py` 의 `BACKBONES` dict 에 한 entry 추가:
   ```python
   "DLinear": BackboneSpec(
       name="DLinear",
       model_factory=lambda cfg: _dlinear.Model(cfg),
       default_model_hps=dict(moving_avg=25),
       default_training_hps=dict(learning_rate=1e-3, batch_size=32, num_epochs=20, patience=3),
       extra_config_fields=["moving_avg"],
   ),
   ```
3. 끝. 다른 코드 무수정.

### 11.5 명령어 (V14 vintage 재현 + 새 backbone 비교)

```bash
cd V13/

# V14 의 iTransformer 결과 그대로 재현 (--backbone 생략 시 default = iTransformer)
python scripts/run_all.py --all-keys --skip-existing --backbone iTransformer
python scripts/run_all.py --analyze        --backbone iTransformer

# 새 backbone (예: DLinear) 추가한 뒤
python scripts/run_all.py --all-keys --skip-existing --backbone DLinear
python scripts/run_all.py --analyze        --backbone DLinear

# Cross-backbone 비교 (per-backbone CSV 가 같이 산출됨)
ls results/04_metrics/
#   per_dataset_metrics__iTransformer.csv
#   per_dataset_metrics__DLinear.csv
```

### 11.6 검증 — V14 결과 invariance

| 검증 항목 | 결과 |
|:--|:--|
| 모든 script 의 정적 import smoke test | ✅ pass |
| Registry → Config → factory pipeline 동작 | ✅ pass (iTransformer 107K params, V14 동일) |
| 200 데이터셋 artifact mv (legacy → new layout) | ✅ 2200 file 이동, leftover 0 |
| PSM `bundle_meta.json` 비교 (refactor 전후) | ✅ bit-for-bit identical |
| PSM 12 metric (`vus_pr_D_w`, `vus_pr_D_w_z`, ...) 비교 | ✅ Δ = 0 (전부) |

§4 의 4-way 결과, §5 의 z_max 우위 분석, §9 의 production 권장 모두 그대로 유효 — 이번 변경은 **코드 organization 한정** 으로 평가 결과에는 영향 없음.

### 11.7 PR 흐름 (이 vintage 의 git history)

| PR | 내용 |
|:--|:--|
| #1 — `refactor/backbone-pluggable` | model/base.py, model/__init__.py registry, config_factory.py 신설 + 02–07 / run_all 의 backbone 인자화 + 200 artifact 일회성 mv |
| #2 — `cleanup/pyrefly-warnings`    | Pyrefly 가 짚은 redundant `int()` / `bool()` 22건 제거 (`np.int64.sum()` 의 `int()` 는 JSON serialize 때문에 그대로 유지), `__getitem__(self, idx)` → `(self, index)` 부모 시그니처 정합 |
| #3 — `docs/backbone-pluggable-readme` | (별도 README PR) |

전부 main 에 merge, feature branch 정리 완료.
